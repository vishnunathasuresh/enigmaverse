...
import { events, EventType, EventStatus } from '../data';
import { Calendar, Filter, X, ExternalLink, Image as ImageIcon, Video, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      <div className="flex flex-col md:flex-row items-center justify-between mb-16 gap-8">
        ...
        <div className="flex flex-wrap items-center gap-3 p-2 rounded-2xl bg-slate-900/50 border border-white/10 backdrop-blur-md">
          ...
          {(['all', 'tech', 'non-tech', 'chai talks'] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={clsx(
      ...
      <div ref={timelineRef} className="relative border-l border-white/10 ml-4 md:ml-8 space-y-12">
        {filteredEvents.map((event, idx) => (
          <div key={event.id} className="timeline-item relative pl-8 md:pl-12 group cursor-pointer" onClick={() => { setSelectedEvent(event); setCurrentPhotoIndex(0); }}>
            {/* Timeline Node */}
            <div className={clsx(
              'absolute left-[-9px] top-2 w-4 h-4 rounded-full border-2 transition-all duration-300',
              event.status === 'upcoming' ? 'bg-cyan-500 border-cyan-300 shadow-[0_0_15px_rgba(6,182,212,0.5)]' :
              event.status === 'current' ? 'bg-blue-500 border-blue-300 shadow-[0_0_15px_rgba(59,130,246,0.5)]' :
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                <div className="flex items-center gap-3">
                  <span className={clsx(
                    ...
                    event.type === 'tech' ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' :
                    event.type === 'non-tech' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' :
                  ...
                  <span className={clsx(
                    'text-sm font-medium',
                    event.status === 'upcoming' ? 'text-cyan-300' :
                    event.status === 'current' ? 'text-blue-300' :
                    'text-slate-500'
                  )}>
                    {event.status}
                  </span>
            ...
            >
              ...
              <div className="relative h-64 md:h-96 bg-black group">
                ...
                {selectedEvent.photos.length > 1 && (
                  <>
                    <button onClick={prevPhoto} className="absolute left-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/50 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80 backdrop-blur-md">
                      <ChevronLeft className="w-6 h-6" />
                    </button>
                    <button onClick={nextPhoto} className="absolute right-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/50 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80 backdrop-blur-md">
                      <ChevronRight className="w-6 h-6" />
                    </button>
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                      {selectedEvent.photos.map((_, i) => (
                        <div key={i} className={clsx('w-2 h-2 rounded-full transition-all', i === currentPhotoIndex ? 'bg-cyan-500 w-6' : 'bg-white/50')} />
                      ))}
                    </div>
                  </>
                )}
              ...
              <div className="p-8">
                ...
                <div className="flex flex-wrap gap-4 pt-6 border-t border-white/10">
                  <a href={selectedEvent.resourcesLink} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-medium transition-colors shadow-lg shadow-cyan-600/20">
                    <ExternalLink className="w-5 h-5" />
                    Resources
                  </a>
                  <a href={selectedEvent.mediaLink} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-white font-medium transition-colors border border-white/10">
